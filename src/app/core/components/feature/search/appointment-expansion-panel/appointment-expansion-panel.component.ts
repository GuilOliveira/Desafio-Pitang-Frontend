...
export class AppointmentExpansionPanelComponent implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		this.subscription = this._appointmentService.deletedAppointment$.subscribe(id => {
			this.handleAppointmentDelete(id);
		});
	}
	...
	handleAppointmentDelete(id: number): void {
		this.allAppointments = this.allAppointments
			.map(appointmentList => appointmentList.filter(appointment => appointment.id !== id))
